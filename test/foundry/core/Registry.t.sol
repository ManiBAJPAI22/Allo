...
contract RegistryTest is Test, RegistrySetup, Native, Errors {
    ...
    function test_createProfile_withLargeInputs() public {
    string memory largeName = new string(1000); // Large name string
    bytes memory largePointer = new bytes(2048); // Large metadata pointer

    Metadata memory largeMetadata = Metadata({protocol: 1, pointer: string(largePointer)});
    ...
    assertEq(profile.name, largeName, "Large name mismatch");
    assertEq(profile.metadata.pointer, largeMetadata.pointer, "Large metadata mismatch");
}



function test_uniqueAnchorGeneration() public {
    string memory name1 = "ProfileOne";
    string memory name2 = "ProfileTwo";

    vm.prank(profile1_owner());
    bytes32 profileId1 = registry().createProfile(nonce, name1, metadata, profile1_owner(), profile1_members());

    vm.prank(profile1_owner());
    bytes32 profileId2 = registry().createProfile(nonce + 1, name2, metadata, profile1_owner(), profile1_members());

    Registry.Profile memory profile1 = registry().getProfileById(profileId1);
    Registry.Profile memory profile2 = registry().getProfileById(profileId2);

    assertNotEq(profile1.anchor, profile2.anchor, "Anchor addresses should be unique");
}

function testRevert_recoverFunds_unauthorizedAccess() public {
    address nonOwner = address(0xAAA);

    vm.prank(nonOwner);
    vm.expectRevert(UNAUTHORIZED.selector);
    registry().recoverFunds(address(token), nonOwner);
}



function test_ownershipTransfer_raceCondition() public {
    bytes32 newProfileId = registry().createProfile(nonce, name, metadata, profile1_owner(), profile1_members());

    vm.prank(profile1_owner());
    registry().updateProfilePendingOwner(newProfileId, profile1_member1());

    vm.prank(profile1_member1());
    registry().acceptProfileOwnership(newProfileId);

    // Check that the previous owner cannot revert ownership
    vm.prank(profile1_owner());
    vm.expectRevert(UNAUTHORIZED.selector);
    registry().updateProfilePendingOwner(newProfileId, profile1_owner());
}


function test_largeNativeTokenRecovery() public {
    uint256 largeAmount = 10**30; // Very large amount
    vm.deal(address(registry()), largeAmount);

    address recipient = address(0xBBB);

    vm.prank(registry_owner());
    registry().recoverFunds(NATIVE, recipient);

    assertEq(recipient.balance, largeAmount, "Recipient balance mismatch");
}
