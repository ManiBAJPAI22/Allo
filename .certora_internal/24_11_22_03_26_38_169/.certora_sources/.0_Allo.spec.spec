...
rule OnlyOwnerCanUpdateConfiguration(method f) filtered { f -> f.selector in 
    updateRegistry(address).selector,
    updateTreasury(address).selector,
    updatePercentFee(uint256).selector,
    updateBaseFee(uint256).selector
} {
    env e;
